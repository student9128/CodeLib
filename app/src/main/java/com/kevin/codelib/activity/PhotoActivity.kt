...
import kotlinx.android.synthetic.main.activity_photo.ivBack
import kotlinx.android.synthetic.main.activity_toggle_view.*
 ...
 * Describe:<br/>
 */
class PhotoActivity : BaseActivity(), OnRecyclerItemClickListener {
    ...
    override fun initView() {
        ...
        val launch = coroutineScope.launch {
            loadAlbum()
            loadX()
            mAlbumDataAdapter = AlbumAdapter(this@PhotoActivity, mAllAlbumDataList)
            rvRecyclerView.adapter = mAlbumDataAdapter
            mAlbumDataAdapter?.setOnItemClickListener(this@PhotoActivity)
...
//            albumFolder.displayName=="全部"
            ...
            intent.putExtra("albumPath",if (currentSelectedAllAlbum) mAllAlbumDataList[position].path else mOtherAlbumDataList[position].path)
